...
class profileController {
  static async getProfiles(req, res) {
    try {
      const validationResult = await validateToken(req.headers.authorization, req.headers.refreshtoken);
      if (validationResult) {
        ...
        const { profiles, lastEvaluatedKey: newLastEvaluatedKey } = await ProfileUseCase.getAllProfiles(
          status,
          limit,
          lastEvaluatedKey
        );
  ...
  static async createProfile(req, res) {
    ...
    if (validationResult) {
      try {
        ...
        const profileExists = await ProfileService.checkProfileExists(profileSlug);
        if (profileExists) {
          return res.send("Profile already exists");
        }
        const result = await createProfileUseCase.createProfile(req);
        res.status(201).json({ message: "Profile submitted successfully.", data: result });
      } catch (error) {
  ...
  static async updateProfileData(req, res) {
    try {
      ...
      const updatedProfile = await ProfileService.updateProfileData(slug, path, updatedData);
  ...
  static async updateProfile(req, res) {
    try {
      ...
      res.status(201).json({ message: "Profile updated successfully.", data: result });
    } catch (error) {
      res.status(error.statusCode || 500).json({ error: error.message, details: error.details });
    }
  }
  ...
  static async recreateProfile(req, res) {
    try {
      ...
      res.status(201).json({ message: "Profile recreated successfully.", data: result });
    } catch (error) {
      res.status(error.statusCode || 500).json({ error: error.message, details: error.details });
    }
  }
