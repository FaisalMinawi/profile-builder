...
jest.mock("../../helpers/createSlug.js", () =>
  jest.fn().mockImplementation((firstName, lastName) => `${firstName.toLowerCase()}-${lastName.toLowerCase()}`)
);
...
describe("slugRepository", () => {
  ...
  it("should retrieve profiles by slug successfully", async () => {
    ...
    const mockData = {
      Items: [{ slug: "talentProfile#john-doe-1" }, { slug: "talentProfile#john-doe-2" }],
    };
