...
const Awards = ({ data }) => {
  if (data.awardsCertificates.length === 0) {
    return (
      <section id="awards" className="container bg-white mx-auto mt-2 py-6 px-4 rounded-lg lg:px-10" data-toggle>
      ...
      <div className="content">
        ...
        <div className="flex flex-wrap gap-4">
          {data?.awardsCertificates?.map((award, index) => (
            /* Award entry */
            <div key={index} className="w-full bg-LightPurple rounded-lg py-6 px-4 lg:w-[248px] lg:h-[248px]">
              <p className="text-base text-EX2 my-4">{award.courseDate || award.gradeScore}</p>
