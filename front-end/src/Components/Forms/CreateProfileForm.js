...
const validateGitHubLink = (_, value) => {
  ...
  return !value || gitHubLinkRegex.test(value)
    ? Promise.resolve()
    : Promise.reject(new Error("Please enter a valid GitHub repository link"));
};

const validateLinkedInLink = (_, value) => {
  const linkedInLinkRegex = /^https:\/\/(www\.)?linkedin\.com\/in\/[A-Za-z0-9_-]+\/?$/;
  return !value || linkedInLinkRegex.test(value)
    ? Promise.resolve()
    : Promise.reject(new Error("Please enter a valid LinkedIn profile link"));
};

const validateGoogleDocsLink = (_, value) => {
  const googleDocsLinkRegex = /^https:\/\/docs\.google\.com\/document\/d\/[A-Za-z0-9_-]+\/?/;
  return !value || googleDocsLinkRegex.test(value)
    ? Promise.resolve()
    : Promise.reject(new Error("Please enter a valid Google Docs link"));
};
  ...
  return (
    <div className="form">
      <Form form={form} size="large" layout="vertical" onFinish={onFinish}>
        <Flex>
          <div className="w-50">
            ...
            >
              <Input placeholder="First name" onBlur={handleBlur} onChange={(e) => setFirstName(e.target.value)} />
            </Form.Item>
          </div>
          <div className="w-50">
            <Form.Item
            ...
            >
              <Input placeholder="Last name" onBlur={handleBlur} onChange={(e) => setLastName(e.target.value)} />
            </Form.Item>
          </div>
        </Flex>
        <Flex>
        ...
        <Flex>
          ...
          <div className="w-50">
            <Form.Item
              ...
              name="profileNumber"
              rules={[
                { validator: validateNamNumber },
                { required: true, message: "Please input a valid profile number!" },
              ]}
            >
          ...
          <div className="w-50">
            ...
            >
              <Dragger {...photoProps}>
                ...
                <p className="ant-upload-text">Click or drag image to this area to upload talent profile image</p>
                <p className="ant-upload-hint">Only PNG, JPEG, JPG, and SVG files are allowed.</p>
              </Dragger>
            </Form.Item>
          </div>
          <div className="w-50">
            ...
            >
              <Dragger {...cvProps}>
                ...
                <p className="ant-upload-text">Click or drag file to this area to upload talent CV file</p>
                <p className="ant-upload-hint">Only PDF, TXT, DOC, and DOCX files are allowed.</p>
              </Dragger>
            </Form.Item>
          </div>
          <div className="w-50">
            <Form.Item label="Upload LinkedIn file" name="linkedInPage">
              <Dragger {...cvProps}>
                ...
                <p className="ant-upload-text">Click or drag file to this area to upload talent LinkedIn file</p>
                <p className="ant-upload-hint">Only PDF, TXT, DOC, and DOCX files are allowed.</p>
              </Dragger>
            </Form.Item>
          </div>
          <div className="w-50">
            <Form.Item label="Upload cover letter" name="coverLetter">
              <Dragger {...cvProps}>
                ...
                <p className="ant-upload-text">Click or drag file to this area to upload talent cover letter</p>
                <p className="ant-upload-hint">Only PDF, TXT, DOC, and DOCX files are allowed.</p>
              </Dragger>
            </Form.Item>
          </div>
      ...
      >
        ...
        <p>
          <strong>Experience level:</strong> {formValues.levelOfExperience}
        </p>
        <p>
          <strong>Target role:</strong> {formValues.targetRole}
        </p>
        <p>
          <strong>LinkedIn URL:</strong> {formValues.linkedInLink}
        </p>
        <p>
          <strong>GitHub URL:</strong> {formValues.githubLink}
        </p>
        <p>
          <strong>Score sheet URL:</strong> {formValues.scoreSheetLink}
        </p>
        <p>
        ...
        <p>
          <strong>Profile number:</strong> {formValues.profileNumber}
        </p>
        <p>
          <strong>Profile image:</strong> {formValues.photo?.file?.name}
        </p>
        <p>
          <strong>CV file:</strong> {formValues.cv?.file?.name}
        </p>
        <p>
          <strong>LinkedIn file:</strong> {formValues.linkedInPage?.file?.name}
        </p>
        <p>
          <strong>Cover letter:</strong> {formValues.coverLetter?.file?.name}
        </p>
      </Modal>
    </div>
  );
