...
const Profiles = () => {
  ...
  const { profiles, fetchProfiles, setProfiles, resetPagination } = useFetchProfiles(status, setPagination, pagination);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (lastProfile) {
        if (lastProfile.status === "processing" || lastProfile.status === "generating") {
          fetchProfiles(true);
        } else if (lastProfile.status === "draft" && lastProfileStatus !== "draft") {
    ...
    if (profileToDelete) {
      axios
        ...
        .then(() => {
          ...
          setProfiles((prevProfiles) => prevProfiles.filter((profile) => profile.slug !== profileToDelete));
        })
        .catch((err) => {
    ...
    <>
      <Tabs activeKey={activeKey} onChange={(key) => handleChangeTab(key)}>
        <TabPane tab="Tech reviewer" key="1">
          <TabContent tabKey="1" profiles={profiles} confirmDelete={confirmDelete} />
        </TabPane>
        <TabPane tab="Business reviewer" key="2">
          <TabContent tabKey="2" profiles={profiles} confirmDelete={confirmDelete} />
        </TabPane>
        <TabPane tab="Recruiter" key="3">
          <TabContent tabKey="3" profiles={profiles} confirmDelete={confirmDelete} />
        </TabPane>
        <TabPane tab="Completed" key="4">
          <TabContent tabKey="4" profiles={profiles} confirmDelete={confirmDelete} />
        </TabPane>
        <TabPane tab="All" key="5">
          <TabContent tabKey="5" profiles={profiles} confirmDelete={confirmDelete} />
        </TabPane>
      </Tabs>
